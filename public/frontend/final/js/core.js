...
Application.Mediator = function() {
	var self = this,
		...
		_u = function() {
			var overloads = {
				"String" : function(channel){
					if(_channels[channel].listeners.hasOwnProperty(this.name))
						delete _channels[channel].listeners[this.name]
				},
				"String,String" : function(channel,subchannel) {
					if(_channels[channel].subchannels[subchannel].listeners.hasOwnProperty(this.name))
						delete _channels[channel].subchannels[subchannel].listeners[this.name]
				}
			}
			var args = [];
			for (var i = 0, j = arguments.length; i < j; i++) {
